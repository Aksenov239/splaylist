...
for((cnt=0;cnt<1;cnt++)) 
do
    ./bin/stats_flexlist_locks.alloc_new.reclaim_none.pool_none.out prefill $prefill presecs 0 secs 100 cops 100 threads 1 zipf 1 >> data_graphics_height_contains/flexlist_contains_height_zipf
    for y in "${yx[@]}" 
    do
        for threads in "${thr[@]}" 
        do
